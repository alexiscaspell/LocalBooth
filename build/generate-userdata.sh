...
append_secondary_disk_cmd() {
    ...
    - |
      ...
      # Detect boot disk: use lsblk -P (key=value pairs) to avoid tree characters
      ...
      # Safety: collect ALL disks used by /target mounts to avoid formatting them
      PROTECTED_DISKS=""
      ...
      for dev in /sys/block/sd* /sys/block/nvme* /sys/block/vd*; do
        [ -d "${dev}" ] || continue
        name=$(basename "${dev}")
        # Skip if this disk is protected (boot/efi/target)
      ...
      if [ -n "${SECOND}" ]; then
        # Final safety check: make sure nothing on this disk is mounted under /target
